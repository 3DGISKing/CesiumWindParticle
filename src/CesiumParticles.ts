import { Cartesian2, Cartesian3, Cartographic, EllipsoidalOccluder, Ellipsoid, Math as CesiumMath, Scene, SceneTransforms } from "cesium";
...
interface Options {
    gfs: GFSRecord[],
    scene: Scene;
    particleOptions: ParticlOptions;
}
...
class CesiumParticles {
    ...
    constructor(options: Options) {
        ...
        this.options = options.particleOptions;
        this.field = this.createFieldFromGFS(options.gfs);
        ...
        const cesiumWidget = this.scene.canvas.parentNode;
        cesiumWidget!.appendChild(this.canvas);

        this.particles = this.prepareParticles();
    ...
    createFieldFromGFS(data: GFSRecord[]) {
    ...
    drawCoordsParticle(particle: Particle, min: number, max: number) {
        ...
        if (this.intersectsCoordinate(target) && particle.age <= this.options.maxAge) {
